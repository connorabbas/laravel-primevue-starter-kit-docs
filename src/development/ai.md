...
Remember this project rule for resources/js/pages/**/*.vue: Type Inertia page props with the starter kit's `SharedPageProps<T>` from `@/types`. Declare page-specific props with `defineProps<SharedPageProps<{ ... }>>()`, using generated `App.Data.*` types for structured payloads and `LengthAwarePaginator<App.Data.FooData>` from `@/types` for paginated Data objects. Use the same `SharedPageProps<T>` composition when supplying a generic to `usePage()`.
